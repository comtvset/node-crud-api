...
## Installation
- clone repository `git clone https://github.com/comtvset/node-crud-api.git`
- `npm i`
...
| `npm run start:prod`  | Make a build and run production server |
| `npm run start:multi` | Run multiple servers with the balancer |
| `npm run lint`        | Run type checks             |
| `npm run test`        | Run tests                   |

### Request examples
- GET `curl 'localhost:{port}/api/users'`
- GET `curl 'localhost:{port}/api/users/<id>'`
- POST `curl 'localhost:{port}/api/users' --data '{"username": "User","age": 35,"hobbies": ["hiking"]}'`
- PUT `curl --request PUT 'localhost:{port}/api/users/<id>' --data '{"email":"example@gmail.com"}'`
- DELETE `curl --request DELETE 'localhost:{port}/api/users/<id>'`
...
- Node.js version 22.14.0 or upper
